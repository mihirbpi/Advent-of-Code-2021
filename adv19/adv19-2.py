...
def manhatt_dist(l1, l2):
    return sum(distance_vector(l1, l2))
                    ...
                    for other_elem in other_rotation:
                        ...
                        if(count >= 12 and other_elem[0] in scanners[0].pt_list):
                            ...
                            if(diff == None and rot_index == None):
                                ...
                                rot_index = j
                                scanner_position_list.append(diff)
...
for i in range(0, len(scanner_position_list)):
    for j in range(0, len(scanner_position_list)):
        dist = manhatt_dist(scanner_position_list[i], scanner_position_list[j])
        if(dist > best_dist):
            best_dist = dist
